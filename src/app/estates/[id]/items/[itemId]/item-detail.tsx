...
interface Item {
  ...
  aiIdentification: { title?: string; description?: string } | null;
  aiValuation: { estimate?: string } | null;
  photos: Photo[];
}
  ...
  return (
    <div>
      ...
      <div className="mt-4 flex items-center gap-2">
        ...
        <StatusBadge status={item.status as "active" | "resolving" | "closed"} />
      </div>

      {/* AI Identification */}
      <div className="mt-4">
        <h2 className="text-xs font-medium text-text-muted">Identification</h2>
        {item.aiIdentification ? (
          <div className="mt-1">
            {item.aiIdentification.title && (
              <p className="text-sm font-medium text-text-primary">
                {item.aiIdentification.title}
              </p>
            )}
            {item.aiIdentification.description && (
          ...
          <p className="mt-1 text-sm text-text-secondary italic">
            Results will appear here after AI analysis.
          </p>
        )}
      </div>
      ...
      {/* AI Valuation */}
      <div className="mt-4">
        <h2 className="text-xs font-medium text-text-muted">Valuation</h2>
        {item.aiValuation?.estimate ? (
          <p className="mt-1 text-sm text-text-primary">{item.aiValuation.estimate}</p>
        ) : (
          <p className="mt-1 text-sm text-text-secondary italic">
            Valuation will appear here after AI analysis.
          </p>
        )}
      </div>
